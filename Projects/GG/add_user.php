...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT userID FROM GG_users WHERE UserID = ?");
    ...
    // Insert new user
    $stmt = $conn->prepare("INSERT INTO GG_users (userID, usernickname, userrole) VALUES (?, ?, ?)");
    $stmt->bind_param("ssi", $userID, $usernickname, $userrole);
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['notification'] = [
            'type' => 'success',
            'message' => "User <strong>$userID</strong> (<strong>$usernickname</strong>) with role <strong>$roleName</strong> added successfully!"
